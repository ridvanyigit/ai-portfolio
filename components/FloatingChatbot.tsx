...
export default function FloatingChatbot() {
  // --- ESKİ DOSYADAN GELEN MANTIK ---
  // Pencerenin açık/kapalı durumunu tutar
  const [open, setOpen] = useState(false); 
  
  // --- YENİ DOSYADAN GELEN MANTIK ---
  ...
  // --- BİRLEŞTİRİLMİŞ RENDER ---
  return (
    <>
      {/* 1. YÜZEN AÇ/KAPA BUTONU (Eski koddan) */}
      <button
        onClick={() => setOpen((o) => !o)}
      ...
      {/* 2. BUTONA BASILINCA GÖRÜNEN YÜZEN PENCERE (Eski kodun çerçevesi, yeni kodun içi) */}
      {open && (
            ...
            {messages.length === 0 ? (
              ...
              messages.map((m, i) => (
                ...
                >
                  ...
                  <div className={`max-w-[85%] rounded-lg p-3 text-sm ${ m.role === "user" ? "bg-indigo-600 text-white rounded-br-none" : "bg-muted text-foreground rounded-bl-none" }`}>
